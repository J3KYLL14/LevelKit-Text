"""Content discovery utilities for the text adventure engine."""
...
IMAGES_MODULE = "text_adventure.assets.images.registry"
SOUNDS_MODULE = "text_adventure.assets.sounds.registry"
...
def load_rooms() -> Dict[str, RoomSpec]:
    ...
    for file_path in sorted(LEVELS_DIR.glob("*_room.py")):
        module_name = _module_name_from_path("text_adventure.levels", file_path)
...
def load_battles() -> Dict[str, BattleSpec]:
    ...
    for file_path in sorted(BATTLES_DIR.glob("*.py")):
        if file_path.name.startswith("__"):
            continue
        module_name = _module_name_from_path("text_adventure.battle_loops", file_path)
        module = import_module(module_name)
